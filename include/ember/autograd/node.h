 ...
 *
 * Along with `Edge`s, `Node`s are the fundamental building blocks of computational 
 * graphs. Each Node represents a derivative function that describes how partial 
 * derivatives are calculated for the inputs of a given differentiable operation.
 * 
 ...
 * derivatives ∂c/∂a and ∂c/∂b. These derivatives indicate how much the output `c` 
 * changes with respect to small changes in `a` and `b`.
 * 
 * The most important component of a Node is its call operator (operator()) which 
 * must be overridden with an implementation that calculates the input gradients
 * for the corresponding forward operation. 
 * 
 * During the backward pass, a Node may *optionally* use a collection of 
 * TensorSnapshots that contain values captured during the forward pass. The number 
 * of calculated input gradients must have a 1:1 correspondence with the number of 
 * input connections represented by the `edges` vector.
 */
struct Node {
    /** 
     ...
     * computational graph.
     * 
     * Each edge represents a connection to an input tensor's gradient computation.
     * The order of edges corresponds to the order of input gradients that will be
     * computed in the operator().
     */
     ...
     *
     * @param output_grad The gradient of the loss with respect to this node's output
     * @return A vector of tensors containing the gradients for each input
     *
     * Example: For a binary addition operation (z = x + y), the corresponding node
     * returns gradients of [1, 1] since ∂z/∂x = 1 and ∂z/∂y = 1, meaning the change 
     * in the output is directly proportional to changes in either input.
     */
     ...
     * 
     * This method is typically called during the construction of the computational
     * graph to establish connections between operations.
     */
