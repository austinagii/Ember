...
/**
 * Caclulate the partial derivative of `input` w.r.t. `output` for an addition
 * operation.
 *
 * This function works by first determining the broadcast shape for the input
 * tensor. It then iterates over each dimension, calculating the scaling factor
 * required for the input in that dimension. These scaling factors are collected
 * into a cumulative product, which is then multiplied by the identity matrix
 * with the same shape as the input to produce `input`'s gradient tensor.
 */
xt::xarray<float> calculate_local_add_gradient(xt::xarray<float> input,
...
std::vector<Tensor> AddBackward::operator()(Tensor output_grad) {
  ...
  auto augend_gradient =
      calculate_local_add_gradient(augend.data_, output_grad.data_);
  auto addend_gradient =
      calculate_local_add_gradient(addend.data_, output_grad.data_);
