...
TEST(TensorDivision, MultidimensionalTensorsCanBeDivided) {
  ...
  EXPECT_TRUE(a.gradient->equals_approx(
      Tensor{{1.0 / 3.0, 1.0 / 3.0}, {1.0 / 2.0, 1.0 / 1.0}}));
  // ∂c/∂b = -a/b²
  EXPECT_TRUE(b.gradient->equals_approx(
      Tensor{{-4.0 / 3.0, -3.0 / 3.0}, {-3.0 / 2.0, -3.0 / 1.0}}));
}
...
TEST(TensorDivision, BroadcastingWorks) {
  ...
  EXPECT_TRUE(
      a.gradient->equals_approx(Tensor{1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0}));
...
TEST(TensorDivision, GradientWithBroadcastAndScalar) {
  ...
  // Expected: ∂(a/s)/∂a = 1/s
  EXPECT_TRUE(a.gradient->equals_approx(
      Tensor{{1.0 / 2.0, 1.0 / 2.0}, {1.0 / 2.0, 1.0 / 2.0}}));

  // Expected: ∂(a/s)/∂s = -a/s²
  Tensor expected_grad_scalar = {-1.0 / 4.0 - 2.0 / 4.0 - 3.0 / 4.0 -
                                 4.0 / 4.0};
